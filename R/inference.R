normTail <- function(m = 0, s = 1, L = NULL, U = NULL, M = NULL, df = 1000, curveColor = 1, border = 1, 
    col = "#569BBD", xlim = NULL, ylim = NULL, xlab = "", ylab = "", digits = 2, axes = 1, detail = 999, 
    xLab = c("number", "symbol"), cex.axis = 1, xAxisIncr = 1, ...) {
    if (is.null(xlim)[1]) {
        xlim <- m + c(-1, 1) * 3.5 * s
    }
    xLab <- match.arg(xLab)
    ...
    if (axes == 1 || axes > 2) {
        if (xLab[1] == "symbol") {
            xAt <- m + (-3:3) * s
            xLab <- expression(mu - 3 * sigma, mu - 2 * sigma, mu - sigma, mu, mu + sigma, mu + 2 * 
                sigma, mu + 3 * sigma)
...
chiTail <- function(U = NULL, df = 10, curveColor = 1, border = 1, col = "#569BBD", xlim = NULL, 
    ylim = NULL, xlab = "", ylab = "", detail = 999) {
    if (U <= 30) {
        xlim <- c(0, 30)
    }
    if (U > 30) {
...
FTail <- function(U = NULL, df_n = 100, df_d = 100, curveColor = 1, border = 1, col = "#569BBD", 
    xlim = NULL, ylim = NULL, xlab = "", ylab = "", detail = 999) {
    if (U <= 5) {
        xlim <- c(0, 5)
    }
    if (U > 5) {
...
inference <- function(y, x = NULL, est = c("mean", "median", "proportion"), success = NULL, order = NULL, 
    ...
    # to subtract parameters nsim: number of simulations level: confidence level, value between 0 and
    # 1
    ...
    # plotting settings
    if (eda_plot == TRUE & inf_plot == TRUE) {
        par(mfrow = c(1, 2), mar = c(4, 2, 0.5, 0.5))
    }
    if (any(c(inf_plot, eda_plot) == FALSE)) {
        par(mfrow = c(1, 1), mar = c(4, 2, 0.5, 0.5))
    }
    
    # error: weird y
    if (length(y) == 1) {
        stop("Sample size of y is 1.", call. = FALSE)
    }
    ...
    if (!is.null(x)) {
        if (length(y) != length(x)) {
            stop("The two variables must be of same length.", call. = FALSE)
        }
    }
    ...
    if (x_type == "numerical") {
        x = as.factor(x)
        x_type = "categorical"
        warning("Explanatory variable was numerical, it has been converted to categorical. In order to avoid this warning, first convert your explanatory variable to a categorical variable using the as.factor() function.", 
            call. = FALSE)
    }
    
    # print variable types
    if (x_type == "only1Var") {
        cat(paste("One", y_type, "variable", "\n"))
    }
    if (x_type == "categorical") {
        cat(paste("Response variable: ", y_type, ", Explanatory variable: ", x_type, "\n", sep = ""))
    }
    ...
    if (length(est) > 1) {
        stop("Missing estimate: mean, median, or proportion.", call. = FALSE)
    }
    ...
    if (is.na(which_type)) {
        stop("Type should be 'ci' or 'ht'.", call. = FALSE)
    }
    ...
    if (type == "ht" & x_type == "only1Var" & is.null(null)) {
        stop("Missing null value.", call. = FALSE)
    }
    ...
    if (type == "ht" & length(alternative) > 1) {
        if (x_levels <= 2 & y_levels <= 2) {
            stop("Missing alternative: less, greater, twosided.", call. = FALSE)
        }
        if (x_levels > 2 | y_levels > 2) {
            if (y_type == "numerical") {
                alternative = "greater"
    ...
    if ((x_levels > 2 | y_levels > 2) & type == "ci") {
        if (y_type == "numerical") {
            stop("Categorical variable has more than 2 levels, confidence interval is undefined, use ANOVA to test for a difference between means.", 
                call. = FALSE)
        }
        if (y_type == "categorical") {
            stop("Categorical variable has more than 2 levels, confidence interval is not defined, use chi-square test of independence.", 
                call. = FALSE)
        }
    }
    ...
    if (y_type == "numerical" & est == "proportion") {
        stop("Variable is numerical, sample statistic cannot be a proportion, choose either mean or median.", 
            call. = FALSE)
    }
    ...
    # error: x variable has more than two levels, but alternative is not defined properly (chi-square
    # and ANOVA)
    ...
    if ((y_type == "categorical" & x_levels == 2 & y_levels == 2) | (y_type == "categorical" & is.null(x))) {
        ...
        if (is.null(success)) {
            ...
            stop(paste("Variable is categorical, specify which level to call success: ", y_level_names[1], 
                " or ", y_level_names[2]), call. = FALSE)
        }
    ...
    if (x_type == "only1var") {
        ...
        if (y_type == "categorical") {
            cat("Single", est, "-- success:", success, "\n")
        }
        
        # set statistic: mean, median, or proportion
        if (y_type == "numerical") {
            statistic = match.fun(est)
        }
        if (y_type == "categorical") {
            statistic = function(x) {
                sum(x == success)/length(x)
            }
        }
        ...
        if (est == "mean") {
            if (eda_plot == TRUE) {
                hist(y, main = "", cex.main = 0.75, xlab = y_name, col = COL[3, 4], ylab = "")
            }
        ...
        if (est == "proportion") {
            if (eda_plot == TRUE) {
                barplot(table(y), main = "", xlab = y_name, col = COL[3, 4])
            }
        ...
        if (method == "simulation") {
            ...
            if (type == "ci") {
                ...
                if (inf_plot == TRUE) {
                  ...
                  if (nsim > 500) {
                    ...
                    hist(sim_dist, xlab = "Bootstrap distribution", main = "", ylab = "", xlim = xlim, 
                      col = COL[1, 2])
                    if (inf_lines == TRUE) {
                      for (i in 1:2) {
                ...
                cat(c(conflevel * 100, "% Bootstrap interval = (", round(ci[1], 4), ",", round(ci[2], 
                  4), ")\n"))
            }
            ...
            if (type == "ht") {
                ...
                smaller_tail = round(min(c(mean(sim_dist <= actual), mean(sim_dist >= actual))), 
                  4)
                ...
                if (inf_plot == TRUE) {
                  ...
                  if (nsim > 500) {
                    counts = hist(sim_dist, plot = FALSE)$counts
                    hist(sim_dist, xlab = "Randomization distribution", main = "", ylab = "", xlim = xlim, 
                      col = COL[1, 2])
                  }
                  if (nsim <= 500) {
                    BHH2::dotPlot(sim_dist, xlim = xlim, pch = 19, col = COL[1, 2], cex = 0.8, axes = FALSE, 
                ...
                if (alternative == "less") {
                  if (actual < null) {
                    cat(paste("p-value = ", smaller_tail, "\n"))
                  }
                  if (actual > null) {
                    cat(paste("p-value = ", 1 - smaller_tail, "\n"))
                  }
                  if (inf_lines == TRUE) {
                    if (nsim > 500) {
                    ...
                    if (nsim <= 500) {
                      lines(x = c(actual, actual), y = c(0, 0.8), col = COL[4], lwd = 2)
                    }
                  }
                }
                if (alternative == "greater") {
                ...
                if (alternative == "twosided") {
                  ...
                  if (actual >= null) {
                    ...
                    if (inf_lines == TRUE) {
                      if (nsim > 500) {
                        lines(x = c(null - temp, null - temp), y = c(0, 1.1 * max(counts)), col = COL[4], 
                          lwd = 2)
                      }
                      if (nsim <= 500) {
                        lines(x = c(null - temp, null - temp), y = c(0, 0.8), col = COL[4], lwd = 2)
                      }
                    }
                  }
                  if (actual < null) {
                ...
                if (inf_lines == TRUE) {
                  ...
                  if (nsim <= 500) {
                    text(x = actual, y = 0.9, paste("observed\n", round(actual, 4)), col = COL[4], 
                      cex = 0.8)
                  }
                }
            }
        ...
        if (method == "theoretical") {
            ...
            if (type == "ci") {
                if (y_type == "numerical") {
                  ...
                  if (est == "mean") {
                    ...
                    if (n < 30) {
                      critvalue = qt((1 - conflevel)/2 + conflevel, df = n - 1)
                    }
                  }
                }
                if (y_type == "categorical") {
                ...
                cat(c(conflevel * 100, "% Confidence interval = (", round(ci[1], 4), ",", round(ci[2], 
                  4), ")\n"))
            }
            ...
            if (type == "ht") {
                if (y_type == "numerical") {
                  ...
                  if (est == "mean") {
                    ...
                    if (n >= 30) {
                      ...
                      smaller_tail = round(min(pnorm(teststat), pnorm(teststat, lower.tail = FALSE)), 
                        4)
                    }
                    if (n < 30) {
                      cat(paste("Test statistic: T =", round(teststat, 3), "\n"))
                      cat(paste("Degrees of freedom: ", n - 1, "\n"))
                      smaller_tail = round(min(pt(teststat, df = n - 1), pt(teststat, df = n - 1, 
                        lower.tail = FALSE)), 4)
                    }
                  }
                }
                ...
                if (alternative == "less") {
                  ...
                  axis(1, at = c(-3, teststat, 0, 3), labels = c(NA, paste(round(actual, 2)), paste(null), 
                    NA))
                }
                ...
                if (alternative == "greater") {
                  ...
                  if (inf_plot == TRUE) {
                    ...
                    axis(1, at = c(-3, 0, teststat, 3), labels = c(NA, paste(null), paste(round(actual, 
                      2)), NA))
                  }
                }
                  ...
                  if (inf_plot == TRUE) {
                    if (actual < null) {
                      ...
                      axis(1, at = c(-3, teststat, 0, -1 * teststat, 3), labels = c(NA, paste(round(actual, 
                        2)), paste(null), paste(round(null + (null - actual), 2)), NA))
                    }
                    if (actual > null) {
                      normTail(L = -1 * teststat, U = teststat, axes = FALSE, col = COL[1, 2])
                      axis(1, at = c(-3, -1 * teststat, 0, teststat, 3), labels = c(NA, paste(round(null - 
                        (actual - null), 2)), paste(null), paste(round(actual, 2)), NA))
                    }
                  }
                }
            }
    ...
    if (x_type == "categorical") {
        ...
        if (y_type == "categorical") {
            ...
            if (x_levels == 2 & y_levels == 2) {
                cat("Difference between two ", est, "s -- success: ", success, "\n", sep = "")
            }
            if (x_levels > 2 | y_levels > 2) {
        ...
        if (x_levels == 2 & (y_type == "numerical" | (y_type == "categorical" & y_levels == 2))) {
            ...
            if (est == "mean") {
                ...
                if (eda_plot == TRUE) {
                  boxplot(y ~ x, xlab = x_name, ylab = y_name, main = "", col = COL[3, 4])
                }
            }
            if (est == "median") {
                for (i in 1:x_levels) {
                  cat("n_", names(by(y, x, length))[i], " = ", by(y, x, length)[i], ", ", sep = "")
                  cat("median_", names(by(y, x, median))[i], " = ", round(by(y, x, median)[i], 4), 
                    ", ", sep = "")
                }
                if (eda_plot == TRUE) {
                  boxplot(y ~ x, xlab = x_name, ylab = y_name, main = "", col = COL[3, 4])
            ...
            if (est == "proportion") {
                ...
                if (eda_plot == TRUE) {
                  mosaicplot(table(x, y), xlab = x_name, ylab = y_name, main = "", col = COL[3, 4])
                }
            }
            ...
            cat("Observed difference between ", est, "s (", levels(x)[1], "-", levels(x)[2], ") = ", 
                round(actual, 4), "\n\n", sep = "")
            
            # save label
            label = paste("Difference in sample ", est, "s", ", ", levels(as.factor(x))[1], "-", 
                levels(as.factor(x))[2], sep = "")
            ...
            if (method == "simulation") {
                ...
                if (type == "ci") {
                  ...
                  for (i in 1:nsim) {
                    sim1[, i] = sample(y[x == order[1]], n1, replace = TRUE)
                  }
                  for (i in 1:nsim) {
                    sim2[, i] = sample(y[x == order[2]], n2, replace = TRUE)
                  }
                  ...
                  if (inf_plot == TRUE) {
                    ...
                    if (nsim > 500) {
                      ...
                      if (inf_lines == TRUE) {
                        for (i in 1:2) {
                          segments(ci[i], 0, ci[i], 0.8 * max(counts), col = COL[4], lwd = 2)
                          text(round(ci[i], 4), max(counts), pos = 1, col = COL[4], round(ci[i], 
                            4))
                        }
                      }
                    }
                    if (nsim <= 500) {
                      BHH2::dotPlot(sim_dist, xlim = xlim, pch = 19, col = COL[1, 2], cex = 0.8, 
                        axes = FALSE, xlab = "Bootstrap distribution")
                      axis(1)
                      if (inf_lines == TRUE) {
                        for (i in 1:2) {
                          segments(ci[i], 0, ci[i], 0.6, col = COL[4], lwd = 2)
                ...
                if (type == "ht") {
                  ...
                  for (i in 1:nsim) {
                    sim[, i] = sample(x, n, replace = FALSE)
                  }
                  ...
                  if (inf_plot == TRUE) {
                    if (nsim > 500) {
                      counts = hist(sim_dist, plot = FALSE)$counts
                      hist(sim_dist, main = "Randomization distribution", xlab = "", ylim = c(0, 
                        1.3 * max(counts)), xlim = c(xmin, xmax), cex.main = 0.75)
                    }
                    if (nsim <= 500) {
                      if (y_type == "numerical") {
                        counts = BHH2::dotPlot(sim_dist, main = "Randomization distribution", xlab = "", 
                          pch = 20, cex = 0.8, xlim = c(xmin, xmax), cex.main = 0.75)$y
                      }
                      if (y_type == "categorical") {
                        counts = table(sim_dist)
                        plot(sim_dist, type = "n", ylim = c(0, max(counts) * 1.3), axes = FALSE, 
                          xlim = c(0.9 * min(sim_dist), 1.1 * max(sim_dist)), main = "Randomization distribution", 
                  ...
                  if (alternative == "less") {
                    ...
                    if (inf_lines == TRUE) {
                      lines(x = c(actual, actual), y = c(0, 1.1 * max(counts)), col = "#569BBD", 
                        lwd = 2)
                    }
                  }
                  if (alternative == "greater") {
                  ...
                  if (alternative == "twosided") {
                    ...
                    if (actual >= null) {
                      temp = actual - null
                      if (inf_lines == TRUE) {
                        lines(x = c(null - temp, null - temp), y = c(0, 1.1 * max(counts)), col = COL[1, 
                          2], lwd = 2)
                      }
                    }
                    if (actual < null) {
            ...
            if (method == "theoretical") {
                ...
                if (type == "ci") {
                  if (y_type == "numerical") {
                    ...
                    if (est == "mean") {
                      ...
                      if (n1 < 30 | n2 < 30) {
                        critvalue = qt((1 - conflevel)/2 + conflevel, df = min(n1 - 1, n2 - 1))
                      }
                    }
                ...
                if (type == "ht") {
                  if (y_type == "numerical") {
                    ...
                    if (est == "mean") {
                      ...
                      if (n1 < 30 | n2 < 30) {
                        ...
                        smaller_tail = round(min(pt(teststat, df = min(n1 - 1, n2 - 1)), pt(teststat, 
                          df = min(n1 - 1, n2 - 1), lower.tail = FALSE)), 4)
                      }
                    }
                  }
                  if (y_type == "categorical") {
                  ...
                  if (alternative == "less") {
                    ...
                    if (inf_plot == TRUE) {
                      normTail(L = teststat, axes = FALSE, col = COL[1, 2])
                      axis(1, at = c(-3, teststat, 0, 3), labels = c(NA, paste(round(actual, 2)), 
                        paste(null), NA))
                    }
                  }
                  # alternative = greater
        ...
        if (x_levels > 2 & y_type == "numerical") {
            ...
            if (method == "theoretical") {
                ...
                if (inf_plot == TRUE) {
                  FTail(F, df_n, df_d, col = COL[1])
                }
            }
            
            if (method == "simulation") {
                # anova output anova_output = anova(lmp(y~x)) print(anova_output, signif.stars=F)
        ...
        if ((x_levels > 2 | y_levels > 2) & y_type == "categorical") {
            ...
            if (method == "theoretical") {
                ...
                print(expected)
                if (any(expected < 5)) {
                  stop("Expected count for at least one cell is less than 5, use method = 'simulation'.", 
                    call. = FALSE)
                }
